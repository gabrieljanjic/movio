    ...
    <div className="p-4 sm:p-6 bg-white">
      ...
      <div className="flex overflow-x-auto gap-3 pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
        {dataRecommendations.results.slice(0, 10).map((series) => {
          return (
            <div
              className="flex-shrink-0 w-[150px] sm:w-[170px] border border-gray-200 bg-gray-50 rounded group"
              key={series.id}
            >
              <Link href={`/series/${series.id}`}>
                ...
                <p className="group-hover:text-blue-500 text-sm sm:text-base text-gray-800 px-2 mt-5 sm:mt-6 pb-2 hover:text-blue-500 transition duration-300 line-clamp-2">
                  {series.name}
                </p>
              </Link>
            </div>
