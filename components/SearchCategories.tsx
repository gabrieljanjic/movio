...
const SearchCategories = async ({
  ...
  const totalUsers = await getUserSearch(query, pageNum);
  const totalResultsUsers = totalUsers.total_results;
  return (
    <div className="mb-3 md:mb-0">
      ...
      <ul className="h-fit bg-white rounded-bl rounder-br">
        ...
        >
          <Link
            href={`/user?page=1&query=${query}`}
            className={type === "user" ? "font-semibold  " : ""}
          >
            <div className="flex justify-between items-center">
              <p className="text-gray-900">User</p>
              <div className="px-2 py-1 bg-slate-100 w-fit flex rounded-xl">
                <span className="text-[12px]">{totalResultsUsers}</span>
              </div>
            </div>
          </Link>
        </li>
